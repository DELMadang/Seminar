...
flowchart TD
    %% 클라이언트 계층
    A1[🖥️ 웹 브라우저] --> B[🚪 API Gateway]
    A2[📱 모바일 앱] --> B
    A3[💻 데스크톱 앱] --> B
    
    %% API Gateway
    B --> |인증/권한 확인<br/>요청 검증<br/>로깅| C[⚖️ 로드밸런서 ]
...
- D2Bridge
  ...
  델파이 Form을 웹으로 전환해주는 등의 기능이 있다. 오픈소스
​
다양한 프레임워크가 존재하니 한번쯤 검토해보는 것도 좋을 듯 하다.

#### 2.2 소스코드 다운로드

델마당 공식 리포지토리에서 클론한다
[델마당 세미나 리포지토리](https://github.com/DELMadang/01.RESTServer)
